...
'栏目ID
t = trim(Cint(request("list")))
'文章ID
id = trim(Cint(request("arc")))
'是否显示该文件名
index = "index.asp"
'查询是否存在该网站配置，否则自动配置
...
elseif  t <> "" and t <> 0  then
 ...
 '获取模板  否则创建模板
  if CheckFile(site_templets+"/css.css") = False then
	...
	Call WriteFile(site_templets +"/stylecss.css",templets_css) 'jan
  end if
 ...
 '栏目名称
  ...
  for key=0 to ubound(cfg_list_arr) 
	  j = key + 1
	  typenames  =  typenames+"<li><a href='"+ geturl + index +"?list="+ Cstr(j) +"' target='_blank'>"+ cfg_list_arr(key) +"</a></li>"
  next 
  '尾部关键词
...
if   id <> "" and id <> 0  then
   ...
   '文章标题
	...
	tname_url = "<a href='"+geturl+index+"?list="+t+"' target='_blank'>"+type_name(1)+"</a>"
	'相关栏目文章
	...
	for i = 1 to max
	    ...
	    if   id <> arc_rand then
	    	id = arc_rand
	    	tag_arc = tag_arc + "<dd><a href='" +geturl+index+ "?list=" +Cstr(t)+ "&arc=" +Cstr(id)+ "' target='_blank'>" +arc_title+ "</a></dd>"
		end if
	next
	'发布
    ...
    for key=0 to ubound(cfg_list_arr) 
	    ...
	    for i = 1 to max
		    ...
		    if   id <> arc_rand then
		    	id = arc_rand
		    	rand_arc = rand_arc + "<dd><a href='" +geturl+index+ "?list=" +Cstr(j)+ "&arc=" +Cstr(id)+ "' target='_blank'>" +arc_title+ "</a></dd>"
			end if
			...
			if   id2 <> arc_rand2 then
				id2 = arc_rand2
				articlelink_all = articlelink_all+ "<a href='" +geturl+index+ "?list=" +Cstr(j)+ "&arc=" +Cstr(arc_rand2)+ "' target='_blank'>" +arc_title2+ "</a>|||"
			end if 
		next
		listlink_all = listlink_all + "<a href='"+ geturl+index+"?list="+Cstr(j)+"' target='_blank'>"+Cstr(cfg_list_arr(j))+"</a>|||"
	next
	'获取文章内容
	...
	'文章url替代
	articleurl = geturl+index+"?list="+Cstr(t)+"&arc="+aid
  ...
  if   t <> "" and t <> 0  then
    ...
    for key=0 to ubound(cfg_list_arr) 
		...
		if  j = Cint(t) then
			...
			desc = title
			tname_url = "<a href='" +geturl+index+ "?list=" +Cstr(j)+ "' target='_blank'>" +cfg_list_arr(key)+ "</a>"
			...
			for i=i to  page_dc
				if   i <> 0 then
				  id = i
 						  arclist = arclist + "<dd><a href='" +geturl+index+ "?list=" +Cstr(j)+ "&arc=" +Cstr(id)+ "' target='_blank'>" +trim(arc_title_arr(id))+ "</a></dd>"
	 			end if
			next
			...
			for i=1 to page_count
				...
				elseif   i = 1 then
					pl = pl + " <a href='" +geturl+index+ "?list=" +Cstr(t)+ "'>" +Cstr(i)+ "</a>"
				else 
...
for key=0 to ubound(cfg_list_arr) 
	...
	typelist =  typelist + "<dt><a href='"+geturl+index+"?list="+Cstr(j)+"' target='_blank'>"+cfg_list_arr(key)+"</a></dt>"
	 count = ubound(arc_title_arr)
	max = 6
	if   count < max then
	 max = count-1
    ...
    for i = 0 to max
	    ...
	    if   id <> 0 and arc_title <> "" then
	    	typelist = typelist + "<dd><a href='" +geturl+index+ "?list=" +Cstr(j)+ "&arc=" +Cstr(id)+ "' target='_blank'>" +arc_title+ "</a></dd>"
		end if
	next
	typelist = typelist + "</dl>"
	max = 20
    ...
    for i = 0 to max
	    ...
	    arc_title = trim(arc_title_arr(id))
	    if   id <> 0 and arc_title <> "" then
	    	sitemap_typelist = sitemap_typelist + "<dd><a href='" +geturl+index+ "?list=" +Cstr(j)+ "&arc=" +Cstr(id)+ "' target='_blank'>" +arc_title+ "</a></dd>"
		end if
	next
next
 '友情链接
