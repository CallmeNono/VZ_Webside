...
Class User_Blog
     ...
     %>                     
		...
		<table width="98%" border="0" align="center" cellpadding="0" cellspacing="1">
			<tr>
			  <td style="font-size:14px">
			  		亲爱的<font color=red><%=KSUser.UserName%></font>用户!您当前可用积分:<font color=red><%=KSUser.GetScore()%></font>分,最多可兑换<font color=blue><%=Cint(KSUser.GetScore()/RS("Score"))%></font>件,如果您确定兑换本礼品,请认真填写以下收货信息,兑换订单一旦提交,便不可取消!              </td>
			</tr>
			<tr>
			...
			<tr>
			    <td  class="splittd" height="35">
				   ...
				   <%dim k,endnum
				   endnum=Cint(KSUser.GetScore()/RS("Score"))
	   ...
	   If RSO.Eof Then
		   ...
		   Call KS.ScoreInOrOut(KSUser.UserName,1,RS("Score")*Amount,"系统","兑换订单号<font color=red>" & OrderID & "</font>的礼品!",0,0)
